...
2. **正取/候補演算法**：會員優先無條件佔額（不受名額限制），零打依報名時間（`created_at`）
   ...
   ✅ **2026-09 第二輪修正（同一條規則的三個漏洞）**：
   - `compute_allocation()` 原本用 `session.get("casual_quota") or 預設值` 補預設，
     `casual_quota = 0`（管理員把零打上限調成 0，代表零打全部候補）會被當成沒設定而變成 15。
     現在只有欄位是 `None` 才套預設值。**以後不要用 `or` 補「0 是合法值」的欄位預設**。
   - `logic.check_and_notify_waitlist()` 原本只用 `total_quota` 判斷遞補，零打名額還是滿的、
     但總名額有空位（例如會員取消）時，會誤發「遞補成功」通知，跟網站實際算出的候補狀態
     兜不起來（跟第一輪修掉的 `webhook.py` 是同類 bug，這支當時漏改）。現在改用
     `compute_allocation()`，並新增 `session`、`old_confirmed` 兩個選填參數：
     `booking_detail.py` 會傳入異動前每筆候補的已正取人數，只有「正取人數增加」才通知，
     避免部分候補的報名在別人異動時被重複通知。
   - `logic.check_and_release_casual_limit()` 沒有跳過會員限定場次，會對零打群發
     「明天場次釋出零打名額」（洩漏會員限定場次的存在與時間，違反第 4／5 點）。現在跳過。
     同時把 `total_quota`／`casual_quota` 讀取改成 `None` 安全（欄位存在但值為 NULL 時，
     `.get(key, default)` 不會用 default，`int(None)` 會讓整個排程中斷）。
...
7. **`sessions.note` 是「自由文字＋系統標記」混用的欄位（脆弱設計，改動要小心）**：
   ...
   **同一個「畫面載入時的舊值蓋掉新值」問題也發生在數字欄位**：Streamlit 輸入框建立後
   `value=` 就不再生效，會一直沿用舊內容。`check_and_release_casual_limit()` 會自動把
   `casual_quota` 調大並發通知，管理員畫面若正開著，按「確認更新」會用舊的零打名額蓋回去，
   等於通知說擴增了、實際名額卻被改回去（標記還在，排程也不會再釋出）。修法：輸入框的 key
   帶入資料庫目前的值（值一變輸入框自動重置成最新值），且儲存時**只寫入管理員真的改過的
   欄位**（跟載入時的基準值比較），沒動的欄位不送出；沒有任何變更會顯示「沒有任何變更」。
   往後在 Streamlit 寫「編輯既有資料」的表單，都要注意輸入框會沿用舊值這件事。
...
- **`admin_sessions.py` 備註標記保護（第 7 點的修正）**：目前只做過語法編譯和
  ...
  ④ 打開編輯區後，讓 `check_and_release_casual_limit()` 自動調大該場 `casual_quota`
  （或直接在資料庫改），回來只改備註按「確認更新」，確認 `casual_quota` 沒被舊值蓋回去。
- **旗標拆成獨立欄位**：見業務規則第 7 點，目前只是止血，`note` 混用問題尚未根治。
- **第二輪修正（`compute_allocation` 的 0 預設值、遞補通知、會員限定釋出名額）**：目前只用
  假環境腳本測過函式行為，尚未在實際環境驗證。建議手動走：① 把某場零打上限調成 0，確認
  網站與 LINE/LIFF 都判定零打全部候補；② 零打名額已滿、總名額有空位時取消一筆會員，確認
  不會發「遞補成功」，把零打上限調大後才會發；③ 把明天的場次設成會員限定，確認排程不會
  對零打群發「釋出零打名額」。
